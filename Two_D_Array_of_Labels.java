//87zMZ_8GwxKl4wRQX8YpExye
  import javax.swing.*;//Import the 3 libraries
import java.awt.*;
import java.awt.event.*;

public class Two_D_Array_of_Labels extends JFrame implements ActionListener   //Class definition
{ 
...
//Declaring a single button
  ...
  JLabel welcomelabel = new JLabel("Welcome to .... Minesweeper Game - Please enter your selection",JLabel.CENTER);
  JLabel statuslabel = new JLabel("Waiting for selection..",JLabel.CENTER);
  
  
  //In the constructor method I am  initializing values
  public Two_D_Array_of_Labels  () //COnstructor method - same name as the Class name
  { 
    ...
    for (int row=0; row<ROW; row++)
    {
      ...
      {
        button_value = row*10+(col+1);//get each buttons value in int form
    ...
    setTitle("2D Array for MineSweeper");
    setSize(800, 600);
    ...
    for (int row=0; row<ROW; row++)
    {
      for (int col=0; col<COL; col++) {
        ...
        ArrayButtonPanel.add(TwoDLabels[row][col]);
      }
    }
